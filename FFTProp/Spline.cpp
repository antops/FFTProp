...
{
	complex<double> SinglePointInterp2D(const vector<double> vx, vector<double> vy, vector<vector<complex<double>>> value, double px, double py) {
		complex<double> result;
		double tempreal, tempimag;
		double* xx = NULL;	xx = new double[vx.size()];	for (int i = 0; i < vx.size(); i++) { xx[i] = vx[i]; }	//采样点坐标-x
		double* yy = NULL;	yy = new double[vy.size()]; for (int j = 0; j < vy.size(); j++) { yy[j] = vy[j]; }	//采样点坐标-y
		double* vyre = NULL;	vyre = new double[vy.size()];
		double* vyim = NULL;	vyim = new double[vy.size()];
		double ** vaxr = NULL;	vaxr = Allocate_2D(vaxr, vy.size(), vx.size());
		double ** vaxi = NULL;	vaxi = Allocate_2D(vaxi, vy.size(), vx.size());
		for (int i = 0; i < vx.size(); i++)
			for (int j = 0; j < vy.size(); j++) {
				vaxr[j][i] = value[i][j].real();
				vaxi[j][i] = value[i][j].imag();
			}
		for (int j = 0; j < vy.size(); j++) {
			Spline SplineRe(xx, vaxr[j], int(vx.size()));
			Spline SplineIm(xx, vaxi[j], int(vx.size()));
			SplineRe.SinglePointInterp(px, vyre[j]);
			SplineIm.SinglePointInterp(px, vyim[j]);
		}
		Spline SplineRe(yy, vyre, int(vy.size()));	SplineRe.SinglePointInterp(py, tempreal);
		Spline SplineIm(yy, vyim, int(vy.size()));	SplineIm.SinglePointInterp(py, tempimag);
		result = complex<double>(tempreal, tempimag);
		delete[] xx;	xx = NULL;		delete[] yy;	yy = NULL;
		delete[] vyre;	vyre = NULL;	delete[] vyim;	vyim = NULL;
		Free_2D(vaxr);	Free_2D(vaxi);	vaxr = NULL;	vaxi = NULL;
		return result;
	}
	//构造函数
